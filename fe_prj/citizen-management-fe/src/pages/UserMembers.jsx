...
export default function UserMembers() {
  ...
  const isHouseholdHead = household && household.chuHo === user?.fullName;
    ...
    const fetchMembers = async () => {
      try {
        ...
        if (user?.householdId) {
          ...
          const householdData = await householdService.getHouseholdByNumber(
            user.householdId
          );
          setHousehold(householdData);

          // Fetch residents
          const residentsData = await residentService.getResidentsByHousehold(
            user.householdId
          );

          // residentsData might be an array or an object with array
  ...
  const handleAddResident = async (newResident) => {
    try {
      ...
      const result = await residentService.addNewResident(newResident);
