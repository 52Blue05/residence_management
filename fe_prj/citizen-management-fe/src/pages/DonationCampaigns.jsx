...
export default function DonationCampaigns() {
  ...
  useEffect(() => {
    const fetchCampaigns = async () => {
      try {
        const data = await donggopService.getAll();
  ...
  const statusConfig = {
    ...
    closed: {
      label: "ĐÃ ĐÓNG",
      color: "bg-gray-500/10 text-gray-300 border-gray-500/40",
    },
  };
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {filtered.map((campaign) => {
                  ...
                  const participationRate =
                    campaign.householdsCount > 0
                      ? (
                          (campaign.participatingCount /
                            campaign.householdsCount) *
                          100
                        ).toFixed(0)
                      : 0;
                  return (
                    <div
                      key={campaign.id}
                      className="bg-gray-900/80 border border-white/5 rounded-3xl p-6 space-y-4"
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {campaign.status === "upcoming" && (
                          <div className="flex gap-2">
                            <button className="px-3 py-1 rounded-lg bg-yellow-500/10 text-yellow-300 border border-yellow-400/30 text-xs">
                              ✏️ Sửa
                            </button>
                            <button className="px-3 py-1 rounded-lg bg-red-500/10 text-red-300 border border-red-400/30 text-xs">
                              🗑️ Xóa
                            </button>
                          </div>
                        )}
                        ...
                        <>
                          <div className="p-4 rounded-xl bg-gray-800/60 border border-gray-700 space-y-2">
                            <p className="text-sm text-gray-300">
                              {campaign.participatingCount} hộ đã đóng /{" "}
                              {campaign.householdsCount} hộ ({participationRate}
                              %)
                            </p>
                            <p className="text-lg font-semibold text-white">
                              Tổng:{" "}
                              {campaign.totalCollected.toLocaleString("vi-VN")}{" "}
                              đ
                            </p>
