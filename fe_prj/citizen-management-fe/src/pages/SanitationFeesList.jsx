...
export default function SanitationFeesList() {
  ...
  const [sanitationFeeRecords, setSanitationFeeRecords] = useState([]);
  const [feeRate, setFeeRate] = useState(6000);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const fees = await phiVeSinhService.getAll();
        setHouseholdRecords(households || []);
        setSanitationFeeRecords(fees || []);
        if (fees && fees.length > 0) {
          setFeeRate(fees[0].rate || 6000);
        }
      } catch (error) {
  ...
  const allRecords = useMemo(() => {
    return householdRecords.map((household) => {
      const feeRecord = sanitationFeeRecords.find(
        (r) => r.householdId === household.id && r.year === year
      );
  ...
  const filtered = useMemo(() => {
    return allRecords.filter((record) => {
      ...
      const matchesArea =
        areaFilter === "all" ? true : Number(areaFilter) === record.area;
      const matchesStatus =
            ...
            <div className="w-full h-full p-6 md:p-8 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-gray-900/80 border border-white/5 rounded-2xl p-5">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {stats.total > 0
                      ? ((stats.unpaid / stats.total) * 100).toFixed(1)
                      : 0}
                    %
                  </p>
                </div>
              ...
              <div className="bg-gray-900/80 border border-white/5 rounded-3xl shadow-2xl">
                <div className="p-6 border-b border-white/5 flex flex-wrap gap-4 items-center">
                  <select
                    ...
                    className="bg-gray-800/80 text-gray-200 px-3 py-2 rounded-xl border border-gray-700"
                  >
                  ...
                  <div className="flex items-center gap-2 bg-gray-800/80 rounded-xl px-3 border border-gray-700 flex-1 min-w-[200px]">
                    ...
                    <input
                      ...
                      placeholder="Tìm tên chủ hộ, số HK..."
                      ...
                      {paginated.map((record, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-6 py-4 text-gray-200">
                              {record.requiredAmount.toLocaleString("vi-VN")} đ
                            </td>
                            <td className="px-6 py-4 text-gray-200">
                              {record.paidAmount.toLocaleString("vi-VN")} đ
                            </td>
                            <td className="px-6 py-4">
                              <span
                            ...
                            <td className="px-6 py-4">
                              <div className="flex justify-center gap-2">
                                <button
                                  className="p-2 rounded-lg bg-blue-500/10 text-blue-300 border border-blue-400/30"
                                  title="Xem chi tiết"
                                >
                                ...
                                <button
                                  onClick={() =>
                                    navigate("/fees/sanitation/collect")
                                  }
                                ...
                                <button
                                  className="p-2 rounded-lg bg-purple-500/10 text-purple-300 border border-purple-400/30"
                                  title="In biên lai"
                                >
                                  🖨️
                                </button>
                                <button
                                  className="p-2 rounded-lg bg-yellow-500/10 text-yellow-300 border border-yellow-400/30"
                                  title="Gửi thông báo"
                                >
                ...
                <div className="p-6 border-t border-white/5 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-3 py-2 rounded-lg bg-gray-800 text-gray-300 disabled:opacity-50"
                    >
                      Trước
                    </button>
